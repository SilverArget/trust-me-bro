...
window.__difficultyRun=(level,part,deathCount)=>{if(!systemPaused)pauseGame('difficulty-test');__tmbSetProgress(level,part,0,0);partDeaths=deathCount;rt=makeRuntime();rt.enemies=[];rt.fish=[];rt.fans=[];rt.launchPads=[];rt.rage.patterns=[];spawnGrace=0;lives=10;const g=buildScene(level,part),trigger=sceneStart()+g.anchors.trap.trigger,hazard=sceneStart()+g.anchors.hazard,solids=solidSurfaces(g.surfaces),probeX=g.anchors.trap.trigger,candidates=solids.filter(s=>probeX+player.w>s.x&&probeX<s.x+s.w),support=candidates.sort((a,b)=>a.y-b.y)[0]||solids.filter(s=>s.x<=probeX).sort((a,b)=>b.x-a.x)[0];player.x=trigger+.01;player.y=(support?.y??GROUND)-player.h;player.vx=255;player.vy=0;player.onGround=true;keys.right=true;keys.jump=true;const startDeaths=deaths;let clear=false;for(let frame=0;frame<180;frame++){update(1/60);if(hurtbox().x>=hazard+104){clear=true;keys.right=false;keys.jump=false;player.x=sceneStart()+66;player.y=GROUND-player.h;player.vx=player.vy=0;player.onGround=true}}keys.right=keys.jump=false;return{clear,dead,extraDeaths:deaths-startDeaths,warning:warningDelay(.8,level,deathCount),patterns:ragePatterns(level,part,deathCount)}};
...
test('death tiers remain escapable 5/5 and reset changes runtime', async ({page}) => {
  await page.goto(base);await page.waitForFunction(()=>window.__tmb?.platform.initialized);if(await page.locator('#characterSelect.show').count())await page.locator('.characterChoice').first().click();
